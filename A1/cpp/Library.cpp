    ...
    public:
        ...
        void DaysPassed(int day)
        {
            for (int idx = 0; idx < this->V_book->size(); idx++)
            {
                ...
                if (book->IsBorrowedB())
                {
                    auto mem = book->Getmem();
                    mem->set_money(mem->GetSavingMoney() - (day * book->GetPrice() / 10));
                }
            }
        }
        bool BorrowBook(Book *b_book, Member *b_member)
        {
            int price = (b_book->GetPrice() / 4);
            if ((b_member->GetSavingMoney()) < price)
                return false;
            if(!b_member->ISregisterd())
                return false;
            if(b_member->IsBorrowed())
                return false;
            if(b_book->IsBorrowedB())
                return false;
            int n_m = (b_member->GetSavingMoney());
            b_member->set_money(n_m - price);
            b_member->Setborrowed(true);
            b_book->Setborrowed(true);
            b_book->Setmem(b_member);
            return true;
        }
